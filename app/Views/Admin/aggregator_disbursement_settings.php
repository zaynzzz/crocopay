...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="bg-white shadow-sm rounded-xl border border-gray-200 p-6">
        <!-- Header -->
        <div class="mb-6">
            <h1 class="text-2xl font-bold text-gray-800 mb-2">Pengaturan Disbursement</h1>
            <p class="text-gray-600">Kelola penyedia disbursement (penarikan dana) yang digunakan dalam sistem.</p>
        </div>

        <form action="/admin/aggregator/disbursement/update" method="post">
            <?= csrf_field() ?>
                ...
                <div class="flex items-center justify-between p-4 border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors">
                    <label class="flex items-center gap-4 w-full cursor-pointer">
                        ...
                        <div class="flex-1">
                            ...
                            <div class="text-sm text-gray-500">Gunakan sebagai disbursement utama</div>
                        </div>
                    </label>
...
document.querySelectorAll('.toggle-btn').forEach(btn => {
    btn.addEventListener('click', async () => {
        ...
        try {
            const res = await fetch(`/admin/aggregator/disbursement/toggle/${name}`, {
                method: 'POST',
                headers: {
            ...
            alert('Gagal mengubah status disbursement.');
            console.error(err);
        }
